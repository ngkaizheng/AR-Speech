        ...
        private SpeakerRecognitionEvent ParseSpeakerRecognitionResponse(IDictionary resp)
        {
            ...
            try
            {
                ...
                foreach (var r in iresults)
                {
                    ...
                    result.confidence = IBM.Cloud.SDK.Utilities.Utility.StringToDouble(iresult["confidence"].ToString());
                    result.final = (bool)iresult["final"];
                    result.from = IBM.Cloud.SDK.Utilities.Utility.StringToDouble(iresult["from"].ToString());
                    result.to = IBM.Cloud.SDK.Utilities.Utility.StringToDouble(iresult["to"].ToString());
                ...
                foreach (var r in iresults)
                {
                    ...
                    if (iwordAlternatives != null)
                    {
                        ...
                        foreach (var w in iwordAlternatives)
                        {
                            ...
                            if (iwordAlternative.Contains("start_time"))
                                wordAlternativeResults.start_time = IBM.Cloud.SDK.Utilities.Utility.StringToDouble(iwordAlternative["start_time"].ToString());
                            if (iwordAlternative.Contains("end_time"))
                                wordAlternativeResults.end_time = IBM.Cloud.SDK.Utilities.Utility.StringToDouble(iwordAlternative["end_time"].ToString());
                            if (iwordAlternative.Contains("alternatives"))
                            {
                                ...
                                foreach (var a in iwordAlternativeResult)
                                {
                                    ...
                                    if (ialternative.Contains("confidence"))
                                        wordAlternativeResult.confidence = IBM.Cloud.SDK.Utilities.Utility.StringToDouble(ialternative["confidence"].ToString());
                    ...
                    if (ialternatives != null)
                    {
                        ...
                        foreach (var a in ialternatives)
                        {
                            ...
                            if (ialternative.Contains("confidence"))
                                alternative.confidence = IBM.Cloud.SDK.Utilities.Utility.StringToDouble(ialternative["confidence"].ToString());
                            ...
                            if (ialternative.Contains("timestamps"))
                            {
                                ...
                                for (int i = 0; i < itimestamps.Count; ++i)
                                {
                                    ...
                                    ts.Start = IBM.Cloud.SDK.Utilities.Utility.StringToDouble(itimestamp[1].ToString());
                                    ts.End = IBM.Cloud.SDK.Utilities.Utility.StringToDouble(itimestamp[2].ToString());
                                    timestamps[i] = ts;
                                }
                            ...
                            if (ialternative.Contains("word_confidence"))
                            {
                                ...
                                for (int i = 0; i < iconfidence.Count; ++i)
                                {
                                    ...
                                    wc.Confidence = IBM.Cloud.SDK.Utilities.Utility.StringToDouble(iwordconf[1].ToString());
                                    confidence[i] = wc;
                                }
                        ...
                        foreach (string keyword in Keywords)
                        {
                            if (iKeywords[keyword] != null)
                            {
                                ...
                                foreach (var k in iKeywordList)
                                {
                                    ...
                                    keywordResult.confidence = IBM.Cloud.SDK.Utilities.Utility.StringToDouble(iKeywordDictionary["confidence"].ToString());
                                    keywordResult.end_time = IBM.Cloud.SDK.Utilities.Utility.StringToDouble(iKeywordDictionary["end_time"].ToString());
                                    keywordResult.start_time = IBM.Cloud.SDK.Utilities.Utility.StringToDouble(iKeywordDictionary["start_time"].ToString());
